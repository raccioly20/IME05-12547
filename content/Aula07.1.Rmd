...
```{r caret, fig.height=4, fig.width=7}
featurePlot(x = conj_treino[ , c("TV", "Radio", "Jornal")], y = conj_treino$Vendas)
```

## Usando o ggplot

```{r ggplot, fig.height=4, fig.width=7}
gt(head(conj_treino, 10))
...
```{r splom, fig.height=6.5, figure.width=7, message=FALSE}
library(psych)
...
```{r residuos1a, message=FALSE, fig.height=6.5, fig.width=9}
plot(mod1)
```

## Análise do modelo 2

```{r residuos1b, message=FALSE, fig.height=7, fig.width=9}
library(performance)
check_model(mod1)
```
...
```{r 2modelo-regressão, fig.height=4, fig.width=4}
...
```{r residuos2, message=FALSE, fig.height=7, fig.width=9}
check_model(mod2)
```
...
```{r 3omodelo-regressao, fig.height=4, fig.width=4}
...
```{r residuos3, message=FALSE, fig.height=6.5, fig.width=9}
check_model(mod3)
```
...
```{r residuos, message=FALSE, fig.height=10, fig.width=9}
check_model(mod5)
```
...
```{r transf_resposta, fig.height=10, fig.width=10}
