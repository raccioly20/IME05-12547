...
ajusreg.comp <- regsubsets(medv ~ ., data=Boston)
...
```{r modelo_completo}
ajusreg.comp <- regsubsets(medv ~ ., data=Boston, nvmax=13)
...
```{r avaliando, fig.height=4.5, figure.width=6.5}
...
```{r FR, fig.height=4.5, figure.width=6.5}
ajusreg.fwd <- regsubsets(medv ~ . , data=Boston,nvmax=13, method="forward")
...
ajusreg.bwd <- regsubsets(medv ~ . , data=Boston,nvmax=13, method="backward")
...
str(conj_teste)
knitr::kable(
  head(conj_treino, 10), booktabs = TRUE,
  caption = '10 primeras linhas do conjunto de treino.'
)
```

## Matriz de correlação

```{r corrplot, fig.height=4.5, figure.width=6.5, message=FALSE}
library(corrplot)
...
```{r BIC, fig.height=4.5, figure.width=6.5}
...
```{r cp_denovo, fig.height=4.5, figure.width=6.5}
...
mod_cp2 <- lm(medv ~ crim + zn + chas + nox + rm + dis + rad + ptratio + black + lstat, data=conj_treino)
summary(mod_cp2)
vif(mod_cp2)
```
...
mod_cp2 <- lm(medv ~ crim + zn + chas + nox + rm + dis + rad + ptratio + black + lstat, data=conj_treino)
summary(mod_cp2)$sigma
summary(mod_cp2)$adj.r.squared
sqrt(mean((conj_teste$medv - predict(mod_cp2, conj_teste)) ^ 2))
