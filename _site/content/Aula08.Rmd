...
set.seed(2021)
indice_teste <- createDataPartition(y, times = 1, p = 0.5, list = FALSE)
...
```{r volta_chute}
p <- 0.9
y_chapeu <- sample(c("Masculino", "Feminino"), 
                   length(indice_teste), replace = TRUE, prob=c(p, 1-p)) %>%                            factor(levels = levels(conj_teste$sexo))
mean(y_chapeu == conj_teste$sexo)
```
